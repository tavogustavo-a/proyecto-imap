    ...
    try:
        ...
        if retention is None or frequency is None:
            ...
            try:
                ...
                # <<< --- DEBUG LOGS AQUÍ --- >>>
                current_app.logger.info(f"[SAVE_SECURITY_SETTINGS] Valores guardados y releídos desde BD:")
                current_app.logger.info(f"    log_retention_minutes: {get_site_setting('log_retention_minutes')}")
                current_app.logger.info(f"    observer_check_frequency: {get_site_setting('observer_check_frequency')}")
                current_app.logger.info(f"    observer_enabled: {get_site_setting('observer_enabled')}")
                # <<< --- FIN DEBUG LOGS --- >>>
...
def clear_trigger_log():
    try:
        ...
        current_app.logger.info(f"Admin (User ID: {session.get('user_id', 'N/A')}) limpió TriggerLog ({num_deleted} filas).")
        # <<< --- DEVOLVER JSON EN LUGAR DE REDIRECT --- >>>
        return jsonify({"status": "ok", "message": f"Log de activadores limpiado ({num_deleted} entradas eliminadas)."})
    except Exception as e:
        db.session.rollback()
        ...
        return jsonify({"status": "error", "message": f"Error al limpiar el log: {str(e)}"}), 500
