...
def create_regex():
    ...
    try:
        ...
        if users_to_update_defaults:
            current_app.logger.info(f"[DefaultAssign] Asignando Regex ID {new_regex.id} como default a {len(users_to_update_defaults)} usuarios con sub-usuarios...")
            ...
            if updated_defaults_count > 0:
                ...
                db.session.commit()
                current_app.logger.info(f"[DefaultAssign] Commit realizado. {updated_defaults_count} usuarios actualizados con default Regex ID {new_regex.id}.")
            else:
                current_app.logger.info(f"[DefaultAssign] No se necesitaron actualizaciones de defaults para Regex.")
        ...
        if all_principal_users:
            current_app.logger.info(f"[AllowedAssignPrincipal] Asignando Regex ID {new_regex.id} a lista allowed de {len(all_principal_users)} usuarios principales...")
            ...
            if updated_allowed_principal_count > 0:
                ...
                db.session.commit()
                current_app.logger.info(f"[AllowedAssignPrincipal] Commit realizado. {updated_allowed_principal_count} usuarios principales actualizados con Regex ID {new_regex.id} en allowed.")
            else:
                 current_app.logger.info(f"[AllowedAssignPrincipal] No se necesitaron actualizaciones de allowed para Regex en principales.")
        ...
        if all_subusers:
            current_app.logger.info(f"[AllowedAssignSubuser] Asignando Regex ID {new_regex.id} a lista allowed de {len(all_subusers)} sub-usuarios existentes...")
            for subuser in all_subusers:
                ...
                if parent and new_regex in parent.regexes_allowed:
                    ...
                    current_app.logger.warning(f"[AllowedAssignSubuser] El padre {parent.id if parent else 'desconocido'} del sub-usuario {subuser.id} no tiene el regex {new_regex.id} permitido. Omitiendo asignación directa al sub-usuario.")

            if updated_allowed_subuser_count > 0:
                # Commit 4 (o 3, o 2): Guardar las nuevas relaciones allowed M2M para sub-usuarios
                db.session.commit()
                current_app.logger.info(f"[AllowedAssignSubuser] Commit realizado. {updated_allowed_subuser_count} sub-usuarios existentes actualizados con Regex ID {new_regex.id} en allowed.")
            else:
                current_app.logger.info(f"[AllowedAssignSubuser] No se necesitaron actualizaciones de allowed para Regex en sub-usuarios existentes.")
