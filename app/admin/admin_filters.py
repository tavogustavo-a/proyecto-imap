...
def create_filter():
    ...
    try:
        ...
        if users_to_update_defaults:
            current_app.logger.info(f"[DefaultAssign] Asignando Filtro ID {new_filter.id} como default a {len(users_to_update_defaults)} usuarios con sub-usuarios...")
            ...
            if updated_defaults_count > 0:
                ...
                db.session.commit() 
                current_app.logger.info(f"[DefaultAssign] Commit realizado. {updated_defaults_count} usuarios actualizados con default Filter ID {new_filter.id}.")
            else:
                 current_app.logger.info(f"[DefaultAssign] No se necesitaron actualizaciones de defaults para Filtro.")
        ...
        if all_principal_users:
            current_app.logger.info(f"[AllowedAssignPrincipal] Asignando Filtro ID {new_filter.id} a lista allowed de {len(all_principal_users)} usuarios principales...")
            ...
            if updated_allowed_principal_count > 0:
                ...
                db.session.commit()
                current_app.logger.info(f"[AllowedAssignPrincipal] Commit realizado. {updated_allowed_principal_count} usuarios principales actualizados con Filter ID {new_filter.id} en allowed.")
            else:
                 current_app.logger.info(f"[AllowedAssignPrincipal] No se necesitaron actualizaciones de allowed para Filtro en principales.")
        ...
        if all_subusers:
            current_app.logger.info(f"[AllowedAssignSubuser] Asignando Filtro ID {new_filter.id} a lista allowed de {len(all_subusers)} sub-usuarios existentes...")
            for subuser in all_subusers:
                ...
                if parent and new_filter in parent.filters_allowed:
                    if new_filter not in subuser.filters_allowed:
                     ...
                     current_app.logger.warning(f"[AllowedAssignSubuser] El padre {parent.id if parent else 'desconocido'} del sub-usuario {subuser.id} no tiene el filtro {new_filter.id} permitido. Omitiendo asignación directa al sub-usuario.")


            if updated_allowed_subuser_count > 0:
                # Commit 4 (o 3, o 2): Guardar las nuevas relaciones allowed M2M para sub-usuarios
                db.session.commit()
                current_app.logger.info(f"[AllowedAssignSubuser] Commit realizado. {updated_allowed_subuser_count} sub-usuarios existentes actualizados con Filter ID {new_filter.id} en allowed.")
            else:
                current_app.logger.info(f"[AllowedAssignSubuser] No se necesitaron actualizaciones de allowed para Filtro en sub-usuarios existentes.")
...
def edit_filter(filter_id):
    ...
    if request.method == "POST":
        ...
        update_filter_service(f, sender, keyword, enabled, cut_after_html)
