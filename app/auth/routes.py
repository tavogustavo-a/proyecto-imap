...
def check_session_revocation():
    """
    1) Verifica si hay que forzar logout global o individual por user_session_rev_count.
    ...
    if "logged_in" in session and "username" in session:
        ...
        global_rev = int(get_site_setting("session_revocation_count", "0"))
        user_rev = session.get("session_revocation_count", 0)

        if user_rev < global_rev:
            session.clear()
            flash("Tu sesión ha sido cerrada globalmente.", "info")
...
def logout():
    ...
    resp = make_response(redirect(url_for("user_auth_bp.login")))
...
def _send_reset_email(to_email, reset_link):
    ...
    try:
        ...
        print(f"Error enviando reset password a {to_email}: {e}")
