/**
 * Limpieza manual y programada del historial de compras (solo admin).
 */
document.addEventListener('DOMContentLoaded', function () {
  const panel = document.getElementById('purchaseHistoryCleanupPanel');
  if (!panel) return;
  ...
  const daysInput = document.getElementById('phCleanupDays');
  const scopeSelect = document.getElementById('phCleanupScope');
  const userWrap = document.getElementById('phCleanupUserWrap');
  const userIdInput = document.getElementById('phCleanupUserId');
  const userSearchInput = document.getElementById('phCleanupUserSearch');
  const userResultsEl = document.getElementById('phCleanupUserResults');
  const userSelectedWrap = document.getElementById('phCleanupUserSelected');
  const userSelectedLabel = document.getElementById('phCleanupUserSelectedLabel');
  const userClearBtn = document.getElementById('phCleanupUserClear');
  let userSearchDebounce = null;
  const USER_SEARCH_MIN_CHARS = 1;
  const USER_SEARCH_MAX_RESULTS = 40;
  const purgeBtn = document.getElementById('phCleanupPurgeBtn');
  const autoEnabled = document.getElementById('phCleanupAutoEnabled');
  const intervalSelect = document.getElementById('phCleanupIntervalHours');
  const saveAutoBtn = document.getElementById('phCleanupSaveAutoBtn');
  const statusEl = document.getElementById('phCleanupStatus');
  const infoBtn = document.getElementById('phCleanupInfoBtn');
  const infoBox = document.getElementById('phCleanupInfoBox');
  ...
  function syncAutoIntervalControls() {
    ...
    if (intervalSelect) {
      ...
      if (!enabled) {
        intervalSelect.title = 'Marca el checkbox para elegir el intervalo';
      } else {
        intervalSelect.title = 'Ejecutar cada';
      }
    }
  }
  ...
  async function purgeNow() {
    ...
    const daysLabel =
      p.retention_days === 0
        ? 'todo el historial de compras (' + scopeLabel + ')'
    ...
    const ok = window.confirm(
      '¿Eliminar ' +
        ...
        daysLabel +
        ')?\n\nLas licencias en inventario no se borran. Esta acción no se puede deshacer.'
    );
    if (!ok) return;
